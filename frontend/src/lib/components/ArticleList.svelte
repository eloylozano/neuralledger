<script>
  ...
  // REACCIÓN MEJORADA: Ahora escucha cambios en cantidad, precio e IVA
  $effect(() => {
    items.forEach((item) => {
      // El total de la línea es la Base Imponible (Cantidad * Precio)
      const base = Number(item.quantity) * Number(item.price);
      item.total_item = base.toFixed(2);

      // Mantenemos la referencia al tax para que el componente reaccione
      // cuando el componente superior cambie el IVA globalmente
      const _triggerIVA = item.tax;
    });
  });
</script>
    ...
    <table>
      <thead>
        <tr>
          ...
          <th width="120" class="text-right">Base Línea</th>
          <th width="50"></th>
        </tr>
      </thead>
      <tbody>
        {#each items as item, i (i)}
          <tr in:fade={{ duration: 200 }}>
            <td class="text-muted text-center font-mono pos-cell"
              >{item.position}</td
            >
...
<style>
  ...
  .subtotal-value {
    ...
    font-size: 0.95rem;
    filter: drop-shadow(
      0 0 8px color-mix(in srgb, var(--primary), transparent 80%)
    );
  }
